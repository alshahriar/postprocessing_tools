...
spd=fft(sig);
[y,ind]=max(abs(spd(1:floor(length(spd)/2))));
...
sig2r=Ar*exp(alphar*(0:n-1)) .* cos(wr*(0:n-1)+phr);
sigr=sig2r;
% resr = [A alpha w ph]; disp(resr)
resr = [Ar -alphar wr/(2*pi) phr]; % disp('A alpha w/(2pi) phi'); disp(resr)
figure(123235);
plot(0:n-1,sig,'r.-',0:n-1,sig2r,'b.-'); legend('Original','Reconstructed')
% plot(0:n-1,sig,'r.-',0:n-1,sig1r,'g.-',0:n-1,sig2r,'b.-'); legend('sig','sig1r','sig2r');
%plot(noise)
end